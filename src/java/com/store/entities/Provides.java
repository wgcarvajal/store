...
@NamedQueries({
    ...
    @NamedQuery(name = "Provides.findProductsByProvId", query = "SELECT NEW com.store.model.ProvidesModel(p,pr,prp) FROM Provides p INNER JOIN Price pr INNER JOIN Pricepurchase prp WHERE  p.provId.provId =:provId And p.providesFinalDate IS NULL AND pr.prodId.prodId = p.prodId.prodId And pr.priceFinalDate IS NULL AND prp.prodId.prodId = p.prodId.prodId And prp.pricePurFinalDate IS NULL AND p.prodId.prodComposition IS NULL ORDER BY p.prodId.prodName asc"),
